...
使用<SKILL: z-git> （如果没有z-git这个skill，可以无视分支管理这部分，或者遵循其他分支管理的skill规范）或其他skill中的git进行分支管理，每次进行集成API test都会基于`<dev-branc>`创建新的`worktree`，`<type>`为`test`。在这个新的`worktree`进行项目开发
...
使用`@general`subagent 编写测试：`worktree`，要求 加载`contract/`目录下的相关contract与`proposal`内容，覆盖所有可能的case。禁止对照`servers/`里的实现来写case，避免出现射箭画靶的情况。
